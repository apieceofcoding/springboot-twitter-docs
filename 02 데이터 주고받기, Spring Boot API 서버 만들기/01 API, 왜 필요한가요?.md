...
Application Programming Interface 의 줄임말이며, 애플리케이션 프로그래밍(우리 서비스)이 외부 손님과 어떻게 의사소통할지 정해주는 명세서(인터페이스)라고 생각하시면 됩니다.

API는 식당의 메뉴판 같은 거예요. 식당에 가서 밥을 먹고 싶을 때, 우리 손님(클라이언트)은 메뉴판(API)에 있는 음식(데이터나 기능)을 주문하죠. 메뉴판에는 어떤 음식을 어떻게 주문할 수 있는지 규칙이 적혀 있고, 종업원은 그 규칙대로 주방(서버)에 요청을 전달해 음식을 가져다줍니다. 

마찬가지로 API는 프로그램끼리 "이런 데이터나 기능을 이런 방식으로 달라"고 요청하고 받을 수 있게 해주는 규칙이에요. 복잡한 주방(서버) 내부를 몰라도 메뉴판(API)만 알면 원하는 걸 얻을 수 있습니다.

![메뉴판은 API](menu-api.png)

"서버가 API 를 제공한다" 라는 말은 곧, "식당(주방)에서 메뉴판을 제공한다" 라고 이해하시면 됩니다.
...
Hypertext Transfer Protocol 의 줄임말로, **웹 브라우저와 서버가 하이퍼텍스트, 즉 웹 페이지의 내용을 주고받을 때 사용하는 규칙**입니다
...
즉, 손님이 주방에게 주문하면 요리를 내어주는 것과 같이, 브라우저(크롬 등)가 서버에게 HTTP 요청을하면 그에 맞는 응답을 해주게 됩니다.
...
REST API는 스마트폰, 태블릿, 웹사이트 등 다양한 기기와 환경에서 동일한 방식으로 사용할 수 있습니다. 이는 개발자들이 한 번 만든 API를 여러 플랫폼에서 재사용할 수 있게 해주어, 개발 효율성을 높여줍니다.
